...
from networking_afc._i18n import _LE, _LI
...
class AristaL3SyncWorker(worker.BaseWorker):
    ...
    def synchronize(self):
        LOG.info(">>>>>>>>>>>>>>>>>>>>>>>>> loop task synchronize")
...
class AsterL3ServicePlugin(service_base.ServicePluginBase,
    ...
    def add_router_interface(self, context, router_id, interface_info):
        ...
        # To create VNet in Aster HW, the segmentation Id is required for this network.
        ...
        try:
            ...
            with excutils.save_and_reraise_exception():
                LOG.error(_LE("Error Adding subnet %(subnet)s to "
                              "router %(router_id)s on Aster HW, Exception =%(exc)s"),
                          {'subnet': subnet, 'router_id': router_id, 'exc': exc})
