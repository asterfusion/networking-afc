...
aster_ext_net_opts = [
    cfg.ListOpt('aster_ext_net_networks',
                ...
                help=_("List of physical_network names with which flat "
                       ...
                       "aster_ext_net networks with arbitrary "
                       "physical_network names. Use an empty list to disable "
                       "aster_ext_net networks."))
]
...
class AsterExtNetTypeDriver(helpers.BaseTypeDriver):
    ...
    def __init__(self):
        ...
        # LOG.debug("ML2 AsterExtNetTypeDriver initialization complete")
        self._parse_networks(
            cfg.CONF.ml2_type_aster_ext_net.aster_ext_net_networks)

    def _parse_networks(self, entries):
        self.aster_ext_net_networks = entries
        if not self.aster_ext_net_networks:
            LOG.debug("aster_ext_net networks are disabled")
        else:
            LOG.debug("Allowable aster_ext_net physical_network names: %s",
    ...
    def validate_provider_segment(self, segment):
        ...
        if not physical_network:
            msg = _("physical_network required for "
                    "aster_ext_net provider network")
            raise exc.InvalidInput(error_message=msg)
        if (self.aster_ext_net_networks is not None and
                not self.aster_ext_net_networks):
            msg = _("aster_ext_net provider networks are disabled")
            raise exc.InvalidInput(error_message=msg)
        if (self.aster_ext_net_networks and
                physical_network not in self.aster_ext_net_networks):
            msg = (_("physical_network '%s' unknown for "
                   "aster_ext_net provider network") % physical_network)
            raise exc.InvalidInput(error_message=msg)

        for key, value in segment.items():
            if value and key not in [api.NETWORK_TYPE,
                                     api.PHYSICAL_NETWORK]:
                msg = _("%s prohibited for aster_ext_net provider "
                        "network") % key
                raise exc.InvalidInput(error_message=msg)
    ...
    def reserve_provider_segment(self, context, segment):
        ...
        LOG.debug("Reserving aster_ext_net network on physical "
    ...
    def release_segment(self, context, segment):
        ...
        physical_network = segment[api.PHYSICAL_NETWORK]
        LOG.debug("Releasing aster_ext_net network on physical "
                  "network %s", physical_network)
